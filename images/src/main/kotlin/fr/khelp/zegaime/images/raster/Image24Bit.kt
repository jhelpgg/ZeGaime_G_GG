...
/**
 * Represents a 24-bit image.
 *
 * **Creation example:**
 * ```kotlin
 * val image = Image24Bit(100, 100)
 * ```
 *
 * **Standard usage:**
 * ```kotlin
 ...
 * @constructor Creates a new 24-bit image.
 */
    ...
    fun parseBitmapStream(inputStream: InputStream)
    {
        ...
        while (y >= 0)
        {
            ...
            while (x < this.width)
            {
                ...
                this.data[x + line] = COLOR_BLACK or
                        (buffer[2].toUnsignedInt() shl 16) or
                        (buffer[1].toUnsignedInt() shl 8) or
